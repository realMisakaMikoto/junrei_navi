...
The bounded workflow-only correction retains the original five tasks in three fresh invocations: tests + `lintRelease`, `assembleRelease`, then debug + instrumentation assembly. Each uses `--no-daemon --max-workers=2` and an explicit 4GB heap. Mandatory audits, SHA/environment guards and material cleanup are unchanged. Official Gradle memory/CLI documentation was checked and all 12 Bash/YAML blocks pass syntax validation. A local single-use `help` invocation independently read a 4096MB heap and two workers and passed in 14s; this verifies resource flags only. Full signed-build success remains unproven: the new workflow/docs commit must pass its own required CI before protected fast-forward and signed retry. Application source and prior local `4def1bc9...` debug-target evidence remain unchanged.
