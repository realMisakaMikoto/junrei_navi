...
Execution update: the protected candidate branch now exists, but signed run `35541425419` failed during SDK setup before signing-material restoration. No keyed artifact came from that attempt; the documented workflow correction must pass new-head normal CI before retry. Actual AMap projection therefore remains outstanding; see the [signed test plan](FRONTEND_SIGNED_TEST_PLAN.md).
