...
The user explicitly approved replacing the unavailable live old-API same-ID comparison with three required evidence layers: **the current primary web parsing contract, native Google and AMap projection/anchor checks, and saved-coordinate regression checks**. The web contract, Google native scope and saved-coordinate regressions have recorded evidence in the [requirement matrix](FRONTEND_REQUIREMENT_MATRIX.md). Actual AMap projection remains required and pending; approval does not complete this revised gate. The original live old/new comparison stays unproven and must never be relabeled as passed. No coordinate correction, new data origin or wider network access is authorized by this baseline decision.

Execution update: signed `3f14597` artifacts passed hash/signer audits and API 37 ARM renders AMap tiles, but that snapshot's three discovery cases still time out; projection/anchors remain unproven. The newer readiness/camera/ABI repair now passes full local build, 417 JVM and fresh audits, plus API 26 artwork/Google/performance regressions. It has no new exact-head CI or signed AMap/API 37 runtime result yet. API 26's compatible-ARM-versus-substitute decision remains unanswered. The revised coordinate gate stays incomplete and original old/new equality stays unproven; see the [signed test plan](FRONTEND_SIGNED_TEST_PLAN.md).
