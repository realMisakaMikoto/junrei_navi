...
- [x] 版权所有者已把未修改的高德 Android 3D 地图 SDK 纳入 `LICENSE` 的窄范围链接/分发例外；该例外不替代高德技术服务许可或第三方条款。
- [x] 中国生产地区数据来自官方或有明确再分发权的渠道，具备来源、版本、许可、源/派生 SHA-256、审图机关、审图号/结论和审核日期。
- [x] Android 与后端生产数据字节/规范化 SHA-256、`regionDataVersion` 和全部权威分类夹具完全一致。
