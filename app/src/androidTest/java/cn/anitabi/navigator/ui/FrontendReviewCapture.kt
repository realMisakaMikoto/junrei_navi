...
internal fun SemanticsNodeInteraction.captureFrontendReview(scene: String, nativeWindow: Boolean = false) {
    ...
    val bitmap = if (nativeWindow) {
        // Compose's dialog capture is unavailable before API 28; capture the actual window on both APIs.
        fetchSemanticsNode()
        val instrumentation = InstrumentationRegistry.getInstrumentation()
        instrumentation.waitForIdleSync()
        requireNotNull(instrumentation.uiAutomation.takeScreenshot())
    } else captureToImage().asAndroidBitmap()
    File(directory, "$scene-api${Build.VERSION.SDK_INT}.png").outputStream().use {
        check(bitmap.compress(Bitmap.CompressFormat.PNG, 100, it))
    }
    if (nativeWindow) bitmap.recycle()
}
