...
class DiscoveryViewModel(
    ...
    init {
        ...
        if (last != null) mutableState.update { it.copy(provider = last.provider, cameraCommand = restore(last)) }
        ...
        viewModelScope.launch {
            repository.state.collectLatest { data ->
                ...
                mutableState.update { current -> current.copy(
                    data = data, mapPoints = prepared?.mapPoints.orEmpty(), pointsById = prepared?.pointsById.orEmpty(),
    ...
    fun initializeLocation(hasPermission: Boolean) {
        ...
        if (hasPermission) locate(initial = true)
    }

    fun locate(initial: Boolean = false) {
        if (state.value.locating) return
        if (!initial) userMovedCamera = true
        val requestedIntent = ++cameraIntent
        mutableState.update { it.copy(locating = true, message = null) }
        viewModelScope.launch {
            val location = try {
                withTimeoutOrNull(8_000) { locationProvider.currentLocation() }
            } catch (exception: CancellationException) { throw exception
            } catch (_: Exception) { null }
            mutableState.update { current ->
                val provider = location?.let(classifyTerritory)?.mapProvider
                val mayMove = requestedIntent == cameraIntent && (!initial || !userMovedCamera)
                current.copy(
                    locating = false, location = location,
                    provider = if (mayMove && provider != null) provider else current.provider,
                    cameraCommand = if (mayMove && location != null && provider != null)
    ...
    fun clearFilters() {
        focusPointIds = null
        mutableState.update { it.copy(filters = emptySet()) }
        savedState["filters"] = longArrayOf()
    }

    fun setVisibleIds(ids: Set<String>) { mutableState.update { it.copy(visibleIds = ids) } }
    fun setBatchMode(enabled: Boolean) { mutableState.update { it.copy(batchMode = enabled) } }
    fun setListMode(enabled: Boolean) { mutableState.update { it.copy(listMode = enabled, message = null) } }
    ...
    fun selectProvider(provider: MapProvider) {
        ...
        mutableState.update { it.copy(provider = provider, visibleIds = emptySet(), listMode = false,
            cameraCommand = DiscoveryCameraCommand.FitAll(++sequence, ids), message = null) }
    }
    fun mapUnavailable() { mutableState.update { it.copy(listMode = true, visibleIds = emptySet(), message = "地图暂时无法加载，已切换到地点列表") } }
    ...
    fun fitAll(pointIds: Set<String>? = null) {
        ...
        mutableState.update { it.copy(
            provider = focus.provider, filters = focus.filters, visibleIds = emptySet(),
    ...
    fun openPoint(id: String, fromMap: Boolean = false) {
        ...
        mutableState.update { current -> current.copy(
            ...
            visibleIds = if (provider != current.provider) emptySet() else current.visibleIds,
    ...
    fun backPanel() { mutableState.update { it.copy(panel = it.panel.back()) }; savePanels() }
    ...
    fun rememberPanel(key: String, presentation: PanelPresentation) {
        mutableState.update { it.copy(panel = it.panel.remember(key, presentation)) }
        savePanels()
    }
    private fun open(panel: DiscoveryPanel) { mutableState.update { it.copy(panel = it.panel.open(panel)) }; savePanels() }
...
internal fun prepareDiscoveryData(
    ...
    val providers = if (sameGeneration) previous!!.providersByPoint else snapshot.points.associate { point ->
        checkCancellation()
        point.id to classifyTerritory(point.coordinate)?.mapProvider
    }
    ...
    // Detail merges can remove folder rows without changing the source generation.
